...
VOID DoSystemOperation( HANDLE hToken ) {
    ...
    if ( !LookupAccountSidA( NULL, pTokenUser->User.Sid, Name, &NameSize, Domain, &DomainSize, &SidType ) ) {
        fprintf( stderr, "[-] LookupAccountSidA failed: %d\n", GetLastError() );
        HeapFree( GetProcessHeap(), 0, pTokenUser );
        return;
    }
...
BOOL ImpersonateSystem( DWORD pid ) {
    ...
    DoSystemOperation( hDupToken );

    if ( !RevertToSelf() ) {
        fprintf( stderr, "[-] RevertToSelf failed: %d\n", GetLastError() );
        CloseHandle( hDupToken );
        CloseHandle( hToken );
        CloseHandle( hProcess );
        return FALSE;
